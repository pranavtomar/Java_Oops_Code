...
class OnlineMart extends Mart{
     ...
     public float findPricePerItem(String itemName) {
         ...
         if(unitPrice == -1.0f){
             ...
             if(onlineDiscountPercentage == -1){
            	 ...
            	 float discount = (onlineDiscountPercentage/100.0f)*unitPrice;
    ...
    public int checkItemAvailability() {
    	...
    	for(int i = 0; i < itemNameArr.length; i++){
    	    if(order.getItemName().equals(itemNameArr[i])){
    	    	if(this.order.getQuantityRequired() <= (itemQuantityArr[i])){
    	    		itemQuantityArr[i] = itemQuantityArr[i] - order.getQuantityRequired();
    ...
    public void shipOrder() {
    	...
    	float pricePerItem = findPricePerItem(getOrder().getItemName());
    	if(quantity == -1 || pricePerItem == -1.0f) {
    		getOrder().setOrderPrice(-1.0);
    		getOrder().setTrackingId("NA");
    	}else {
    		getOrder().setOrderPrice(quantity*pricePerItem);
